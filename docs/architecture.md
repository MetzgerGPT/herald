...
Herald is a bespoke voice AI orchestration platform. It runs a persistent server on a Mac (or Mac mini), connects via WebSocket to a mobile client (iOS, Phase 2+), and routes audio through a local-first pipeline: whisper.cpp → Ollama → Kokoro TTS.
...
| STT | whisper.cpp | Metal-accelerated, ~10× real-time on Apple Silicon |
...
- **Tier 2**: full raw JSON stored in `OpenAILLMContext`; LLM retrieves via tool call (Phase 4)
...
- Same applies to STT: whisper.cpp local → Deepgram cloud (opt-in only)
...
| 1 | Local voice loop — CLI test (no iPhone) | ⬜ Next |
...
```
Mac (herald-server)
    Tailscale IP: 100.x.x.x (or hostname: server.tail-xxxxx.ts.net)
...
| Mode | Target | Notes |
|------|--------|-------|
| Local only | 1.5–2.5s | VAD → whisper → Ollama → Kokoro |
